    ...
    try:
        with ZipFile(artifact) as wheel:
            ...
            installed_product_members = {
                ...
                if not _is_installer_metadata(member)
                and not _is_compiled_bytecode(member)
            }
...
def _is_compiled_bytecode(member: str) -> bool:
    """Installer-generated bytecode caches are not wheel product members.

    ``pip`` records compiled ``__pycache__`` rows in RECORD without digests.
    A cache row is only importable through its wheel-shipped ``.py`` member,
    which is still byte-compared, so dropping the rows loses no guarantee.
    """
    path = PurePosixPath(member)
