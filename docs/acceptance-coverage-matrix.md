...
| `core.lifecycle.telemetry` | Runtime Adapter | `core-lifecycle`; `m_agent.adapters.JsonlTelemetrySink` | Closed JSONL events reconcile public Step/Attempt/Checkpoint identities, preserve lifecycle ordering, Adapter-reported usage provenance, structured failure/error code, Model purpose, nonnegative duration, child-process readability, concurrent writer integrity, and actual canary redaction; a missing dimension, payload-bearing, or uncorrelated event is `FAIL`. | Public event-contract digest and telemetry JSONL digest covering success, failure, child-process, and concurrent streams. | CONTRACT / Foundation | Does not claim external Collector behavior or replace Store authority. |
| `core.lifecycle.telemetry-host` | Runtime Adapter | Isolated `python -I`; `m_agent.adapters.JsonlTelemetrySink` | A wheel-installed host repeats the JSONL lifecycle, failure, close/flush, cross-process, concurrency, duration, purpose, permission-boundary, and redaction checks; malformed or missing host evidence is a Harness error. | Isolated host observation digest plus independently hashed JSONL evidence. | HOST / Foundation | Does not claim external Collector behavior or replace Store authority. |
...
| `core.lifecycle.host-wheel` | Testing | Clean external venv; `python -I -m m_agent.testing` | Exact built wheel and sdist install without editable mode, source path injection, or private imports; changed source/artifact/fixture/environment identity is rejected. | Wheel and sdist SHA-256, all installed wheel members, packaged fixture bytes, build-tool version, Testing dependency summary, source-subject summary, and isolated SQLite restart observation. | HOST / Foundation | Does not prove live provider behavior. |
