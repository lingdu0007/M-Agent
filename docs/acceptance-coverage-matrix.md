...
| `core.lifecycle.telemetry` | Runtime Adapter | `core-lifecycle`; `m_agent.adapters.JsonlTelemetrySink` | Closed JSONL events retain lifecycle ordering, Adapter-provided usage, child-process readability, concurrent writer integrity, and canary redaction; a missing dimension, payload-bearing, or uncorrelated event is `FAIL`. | Public event-contract digest and isolated wheel-process JSONL digest. | CONTRACT / Foundation | Does not claim external Collector behavior or replace Store authority. |
