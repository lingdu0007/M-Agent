...
| `core.lifecycle.host-wheel` | Testing | Clean external venv; `python -I -m m_agent.testing` | Exact built wheel installs without editable mode, source path injection, or private imports; changed source/artifact/fixture/environment identity is rejected. | Wheel-byte SHA-256, all installed wheel members, packaged fixture bytes, build-tool version, runtime dependency summary, and isolated wheel-process observation. | HOST / Foundation | Does not prove live provider behavior. |
