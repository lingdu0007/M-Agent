...
| --- | --- | --- |
| `m_agent.AgentDefinition`, `m_agent.Clock`, `DefinitionRegistry`, `DefinitionSnapshot`, `RetryPolicy`, `Runner`, `SyncRunner`, `RunRecord`, `RunInspection`, `RunStatus`, `RunStore`, `RunLease` | `m_agent.runtime` | Root facade retains only the high-frequency subset above. |
...
| `m_agent.DeterministicModelAdapter`, `DeterministicStreamingModelAdapter`, `DeterministicContextProvider`, `DeterministicTool`, `InMemoryRunStore`, `SQLiteRunStore`, `JsonlTelemetrySink`, `OpenTelemetryTelemetrySink`, `InMemorySpanExporter`, `PlaintextPayloadCodec`, `FakeClock`, `SystemClock` | `m_agent.adapters` | Concrete Adapter implementations move out of Core; the local exporter is CONTRACT evidence, not a Collector. |
