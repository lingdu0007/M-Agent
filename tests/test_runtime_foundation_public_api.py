...
class LayeredRuntimePublicApiTests(unittest.TestCase):
    ...
    def test_bundle_verification_detects_tampering_and_sensitive_views(self) -> None:
        ...
        schema_tampered = bundle.model_copy(update={"schema_version": "2"})
        with self.assertRaises(BundleIntegrityError):
    ...
    def test_bundle_rejects_unstructured_evidence_and_check_text(self) -> None:
        ...
        with self.assertRaises(ValueError):
            ScenarioEvidenceBundle.create(
                ...
                evidence_view={"apiKey": "must not persist"},
                independent_evidence={},
            )
    ...
    def test_cli_assigns_a_distinct_exit_code_to_tampered_bundle(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            bundle["evidence_view"]["run_status"] = "FAILED"
    ...
    def test_cli_rejects_an_incomplete_foundation_manifest(self) -> None:
        ...
        self.assertEqual(completed.returncode, 2)
        self.assertIn("invocation error", completed.stderr)
    ...
    def test_offline_cli_runs_inspects_verifies_and_renders_core_lifecycle(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            bundle_path = root / "bundle.json"
            manifest_path.write_text(manifest.model_dump_json())
            command = [sys.executable, "-m", "m_agent.testing"]
            run = subprocess.run(
                [*command, "run", "--manifest", str(manifest_path), "--output", str(bundle_path)],
                text=True,
                capture_output=True,
            )
            ...
            self.assertEqual(json.loads(inspect.stdout)["scenario"], "core-lifecycle")
